...
- при равном `playersCount` сравниваются `matchesCount`, затем дата, затем `id`;
...
- при равном `playersCount` сравниваются `matchesCount`, затем название;
- `bestPerformingDeck` выбирается только среди колод с
  `isSmallSample = false`;
- для `bestPerformingDeck` сравниваются `matchWinRate`, затем `matchesCount`,
  затем `playersCount`, затем название;
