...
export function analyzePlayerDecksInput(value: string): PlayerDecksInputAnalysis {
  ...
  if (numberedLines.some(Boolean)) {
    const hasContinuousNumbering = numberedLines.every(
      (match, index) => Number(match?.[1]) === index + 1,
    );

    if (!hasContinuousNumbering) {
      errors.push('Нумерация должна идти подряд от 1 без пропусков и повторов.');
    }
  }
  ...
  if (mode === 'named') {
    lines.forEach((line) => {
      ...
      if (!separator || separator.index === undefined) {
        errors.push(
          `Строка ${line.lineNumber}: используйте формат «Имя игрока - Название колоды».`,
        );
        return;
      }

      const playerName = valueWithoutNumber.slice(0, separator.index).trim();
      const deckName = valueWithoutNumber
